// Vendored, generated SCIP protobuf TypeScript bindings (XSPEC-333 R3 Java
// PoC). Regenerated directly from upstream `scip.proto` (also vendored
// alongside this file, unmodified, as `scip.proto`) via the SAME pipeline
// `@c4312/scip` (the R3 C# PoC's original, single-maintainer, frozen-at-0.1.0
// npm dependency — see git history, e.g. `f6c1003`) itself used to build:
// `curl .../scip.proto` -> `buf generate` (protoc-gen-es v2, target=ts) ->
// commit the output. `@c4312/scip` was dropped as a dependency (its README
// scripts are the source of this regen recipe) because its 0.1.0 was
// generated from an older `scip.proto` snapshot (pre-2024-08) that predates
// the `typed_range`/`typed_enclosing_range` `oneof` fields (`SingleLineRange`
// / `MultiLineRange`, replacing the deprecated `repeated int32 range`/
// `enclosing_range`) — verified empirically: `scip-java` 0.13.1 (this Java
// PoC's indexer) emits ONLY the new typed fields, never the deprecated ones,
// so the old bindings silently parsed every Java occurrence's position as
// empty (`range: []`), which would have made row-containment resolution
// (`scip-ingest.ts`) resolve zero definitions/calls for Java. `scip-dotnet`
// (the C# PoC's indexer) happens to still emit the deprecated legacy
// encoding, which is why the original C# PoC's PoC never surfaced this gap.
// See `occurrenceRange`/`occurrenceEnclosingRange` in `scip-reader.ts`, which
...
//   version: v2
//   inputs:
//     - proto_file: ./scip.proto
//   plugins:
//     - local: protoc-gen-es
//       opt: target=ts
//       out: ./
//   YAML
//   PATH="$PWD/node_modules/.bin:$PATH" npx buf generate
//   # then copy scip_pb.ts + scip.proto over these two files.
//
